...
def second(name,Game2):
    #退出二级大厅
    def fun():
        s.sendto(('B ' + name + ' ' +'Game2').encode(),ADDR)
        Game2.config(state=ACTIVE)
        root.destroy()

    #开启单人游戏
    def game2_sig():
        # root.destroy()
        game2_single.play_single()
    ...
    Single=Button(frame1,text='独自玩耍',font=('宋体',40),command=game2_sig)
    Single.pack(padx=105,side=LEFT)
    Online=Button(frame1,text='匹配玩家',font=('宋体',40),command=game2_mul)
...
def receive(name):
    ...
    while True:
        s.sendto(("S %s Game2"%name).encode(),ADDR)
        sleep(0.2)
